...
class DocumentBase(BaseModel):
    ...
    categoria: DocumentCategory = Field(DocumentCategory.OUTROS, description="Document category")
    tipo_documento: DocumentType = Field(DocumentType.MANUAL, description="Document type")
    ...
    file_path: Optional[str] = Field(None, description="File path or URL", max_length=500)
...
class DocumentResponse(DocumentBase):
    ...
    search_vector: Optional[str] = Field(None, description="Search vector representation")
...
class DocumentSearchRequest(BaseModel):
    ...
    categoria: Optional[DocumentCategory] = Field(None, description="Filter by category")
    status: Optional[DocumentStatus] = Field(None, description="Filter by status")
    tipo_documento: Optional[DocumentType] = Field(None, description="Filter by document type")
...
class DocumentUploadRequest(BaseModel):
    ...
    autor: str = Field("", description="Author", max_length=200)
    tags: List[str] = Field(default_factory=list, description="Document tags")
    auto_process: bool = Field(True, description="Auto-process with OCR/text extraction")
    chunk_size: int = Field(1000, description="Text chunk size for processing", ge=100, le=5000)
...
class DocumentBulkOperation(BaseModel):
    ...
    document_ids: List[str] = Field(..., description="Document IDs", min_items=1, max_items=100)
    operation: str = Field(..., description="Operation type (delete, archive, activate)")
    
    @field_validator("operation")
    @classmethod
    def validate_operation(cls, v: str) -> str:
        """Validate operation type."""
        allowed_operations = ["delete", "archive", "activate", "change_category", "add_tags", "remove_tags"]
...
class DocumentBulkUpdate(DocumentBulkOperation):
    ...
    categoria: Optional[DocumentCategory] = Field(None, description="New category for change_category operation")
