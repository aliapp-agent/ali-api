...
class PostgresSessionRepository(SessionRepositoryInterface):
    ...
    async def create(self, session: SessionEntity) -> SessionEntity:
        ...
        try:
            ...
            self.db_session.rollback()
            logger.error(
                "session_creation_failed", error=str(e), user_id=session.user_id
            )
    ...
    async def get_by_id(self, session_id: str) -> Optional[SessionEntity]:
        ...
        try:
            ...
            logger.error(
                "get_session_by_id_failed", error=str(e), session_id=session_id
            )
    ...
    async def update(self, session: SessionEntity) -> SessionEntity:
        ...
        try:
            ...
            logger.info(
                "session_updated", session_id=session.id, user_id=session.user_id
            )
    ...
    async def get_user_sessions(
        ...
        try:
            ...
            raise RepositoryError(
                f"Database error during user sessions lookup: {str(e)}"
            )
    ...
    async def count_user_sessions(
        ...
        try:
            ...
            raise RepositoryError(
                f"Database error during user sessions count: {str(e)}"
            )
    ...
    async def get_active_sessions(
        ...
        try:
            ...
            raise RepositoryError(
                f"Database error during active sessions lookup: {str(e)}"
            )
    ...
    async def get_inactive_sessions(
        ...
        try:
            ...
            raise RepositoryError(
                f"Database error during inactive sessions lookup: {str(e)}"
            )

    async def record_session_message(
        self, session_id: str, tokens_used: int, response_time: float
    ) -> bool:
        ...
        try:
            ...
            self.db_session.rollback()
            logger.error(
                "record_session_message_failed", error=str(e), session_id=session_id
            )
            return False

    async def bulk_update_status(
        self, session_ids: List[str], status: SessionStatus
    ) -> int:
        ...
        try:
            ...
            logger.info(
                "bulk_session_status_update", count=affected_rows, status=status.value
            )
    ...
    async def get_sessions_by_type(
        self, session_type: SessionType, limit: int = 50
    ) -> List[SessionEntity]:
        """Get sessions by type (simplified implementation)."""
        return []

    async def update_session_activity(
        self, session_id: str, activity_data: Dict
    ) -> bool:
