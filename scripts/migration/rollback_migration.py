...
class FirebaseRollbackService:
            ...
            for session in sessions:
                ...
                for message in messages:
                    ...
                    if batch_count >= 500:
                        ...
                        batch_count = 0
                        logger.info(
                            f"Deleted {self.rollback_stats['messages_deleted']} messages..."
                        )
        ...
        try:
            ...
            for session in sessions:
                ...
                if batch_count >= 500:
                    ...
                    batch_count = 0
                    logger.info(
                        f"Deleted {self.rollback_stats['sessions_deleted']} sessions..."
                    )
    ...
    async def rollback_firebase_users(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"✅ Processed {len(self.migrated_user_uids)} Firebase Auth users"
            )
    ...
    async def rollback_specific_user(self, user_email: str) -> bool:
        ...
        try:
            ...
            users_ref = self.firestore_db.collection("users")
            users_query = users_ref.where("email", "==", user_email).where(
