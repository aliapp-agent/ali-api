...
from app.domain.entities import MessageEntity, MessageRole, MessageStatus
...
class PostgresMessageRepository(MessageRepositoryInterface):
    ...
    async def get_by_id(self, message_id: str) -> Optional[MessageEntity]:
        ...
        try:
            ...
            logger.error("get_message_by_id_failed", error=str(e), message_id=message_id)
    ...
    async def update(self, message: MessageEntity) -> MessageEntity:
        ...
        try:
            ...
            logger.info("message_updated", message_id=message.id, session_id=message.session_id)
    ...
    async def get_session_messages(
        ...
        try:
            ...
            query = query.offset(offset).limit(limit).order_by(MessageModel.created_at.asc())
            ...
            logger.error("get_session_messages_failed", error=str(e), session_id=session_id)
            raise RepositoryError(f"Database error during session messages lookup: {str(e)}")
    ...
    async def get_user_messages(
        ...
        try:
            ...
            # Apply pagination and ordering
            query = query.offset(offset).limit(limit).order_by(MessageModel.created_at.desc())
            ...
            raise RepositoryError(f"Database error during user messages lookup: {str(e)}")
    ...
    async def get_conversation_context(
        ...
        try:
            ...
            if before_message_id:
                ...
                if before_message:
                    query = query.where(MessageModel.created_at < before_message.created_at)
            ...
            logger.error("get_conversation_context_failed", error=str(e), session_id=session_id)
    ...
    async def get_message_statistics(
        ...
        try:
            ...
            error_messages = self.db_session.exec(
                select(func.count(MessageModel.id)).where(
                    and_(
                        MessageModel.status == MessageStatus.ERROR.value,
                        *filters
                    )
                )
            ).one()
            ...
            token_stats = self.db_session.exec(
                select(
                    func.sum(MessageModel.metadata["tokens_used"].astext.cast(text("integer"))),
                    func.avg(MessageModel.metadata["tokens_used"].astext.cast(text("integer")))
                ).where(
                    and_(
                        MessageModel.role == MessageRole.ASSISTANT.value,
            ...
            response_time_stats = self.db_session.exec(
                select(
                    func.avg(MessageModel.metadata["processing_time"].astext.cast(text("float")))
                ).where(
                    and_(
                        MessageModel.role == MessageRole.ASSISTANT.value,
    ...
    async def get_conversation_export_data(
        ...
        try:
            ...
            logger.error("get_conversation_export_data_failed", error=str(e), session_id=session_id)
        ...
        try:
            ...
            result = self.db_session.exec(
                ...
                """.format(
                    ...
                    if exclude_session_ids else ""
                )),
                {
                    "older_than": older_than,
                    "updated_at": datetime.utcnow(),
                    **({"exclude_session_ids": tuple(exclude_session_ids)} if exclude_session_ids else {})
                }
            )
    ...
    async def get_token_usage_by_period(
        ...
        try:
            ...
            query = select(
                ...
                func.sum(MessageModel.metadata["tokens_used"].astext.cast(text("integer"))).label("tokens")
            ).where(
                and_(*filters)
            ).group_by(
    ...
    def _entity_to_model(self, entity: MessageEntity) -> MessageModel:
        ...
        return MessageModel(
            ...
            message_metadata=str(entity.metadata.to_dict()) if entity.metadata else None,
    ...
    def _update_model_from_entity(self, model: MessageModel, entity: MessageEntity) -> None:
        ...
        model.message_metadata = json.dumps(entity.metadata.to_dict()) if entity.metadata else None
        model.context = json.dumps(entity.context) if entity.context else None
        model.updated_at = entity.updated_at

    # Additional methods to satisfy the interface
    async def bulk_update_status(self, message_ids: List[str], status: MessageStatus) -> int:
    ...
    async def get_high_token_messages(self, token_threshold: int, limit: int = 50) -> List[MessageEntity]:
        """Get high-token messages (simplified implementation)."""
        return []
    
    async def get_messages_by_status(self, status: MessageStatus, limit: int = 50) -> List[MessageEntity]:
        """Get messages by status (simplified implementation)."""
        return []
    
    async def get_recent_messages(self, hours: int = 24, limit: int = 50) -> List[MessageEntity]:
