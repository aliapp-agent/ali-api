...
async def verify_token(request: Request, user: dict = Depends(firebase_auth_required)):
    ...
    try:
        ...
        return UserProfileResponse(
            ...
            created_at=(
                user_profile.get("created_at").isoformat()
    ...
    try:
        ...
        if profile_data.display_name is not None:
            await firebase_auth_service.update_user(
                user_id, display_name=profile_data.display_name
            )
...
async def resend_verification_email(
    request: Request, user: dict = Depends(firebase_auth_required)
):
    ...
    try:
        ...
        verification_link = (
            await firebase_auth_service.generate_email_verification_link(email)
        )
