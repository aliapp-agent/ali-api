...
class FirestoreMessageRepository(BaseFirestoreRepository, MessageRepositoryInterface):
    ...
    async def get_message_by_id(self, session_id: str, message_id: str) -> Optional[MessageEntity]:
    ...
    async def get_messages_by_role(
        ...
        query = messages_collection.where('role', '==', role).order_by('timestamp').limit(limit)
        docs = query.stream()
    ...
    async def get_messages_since(
        ...
        query = messages_collection.where('timestamp', '>=', since).order_by('timestamp').limit(limit)
        docs = query.stream()
    ...
    async def get_user_messages(
        ...
        for session_id in sessions_to_search:
            messages_collection = self.get_messages_collection(session_id)
            query = messages_collection.where('user_id', '==', user_id)\
    ...
    async def get_messages_by_status(
        ...
        query = messages_collection.where('status', '==', status)\
                                 .order_by('timestamp')\
                                 .limit(limit)
    ...
    async def get_high_token_messages(
        ...
        if session_id:
            ...
            sessions_query = self.collection.order_by('updated_at', direction='desc')\
                                          .limit(50)
            sessions_docs = list(sessions_query.stream())
            sessions_to_search = [doc.id for doc in sessions_docs]
        ...
        for session_id in sessions_to_search:
            ...
            docs = list(messages_collection.order_by('timestamp', direction='desc')
                       .limit(100).stream())
    ...
    async def archive_old_messages(
        ...
        for session_id in sessions_to_process:
            ...
            query = messages_collection.where('timestamp', '<', cutoff_date)\
    ...
    async def cleanup_deleted_messages(
        ...
        for session_id in sessions_to_process:
            ...
            query = messages_collection.where('status', '==', 'deleted')\
                                     .where('updated_at', '<', deleted_before)
