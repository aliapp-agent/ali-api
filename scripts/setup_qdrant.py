...
class QdrantSetup:
    ...
    async def check_connection(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✅ Connected to Qdrant - {len(info.collections)} existing collections"
            )
            return True
        except Exception as e:
    ...
    async def create_collection(
        ...
        try:
            ...
            logger.info(
                f"✅ Created collection '{collection_name}' - {config['description']}"
            )
            return True
    ...
    async def verify_setup(self) -> None:
        ...
        try:
            ...
            for collection in collections.collections:
                ...
                logger.info(
                    f"  - {collection.name}: {info.vectors_count} vectors, {info.status}"
                )
    ...
    async def recreate_collection(self, collection_name: str) -> bool:
        ...
        return await self.create_collection(
            collection_name, self.collections_config[collection_name]
        )
    ...
    async def add_sample_data(self) -> None:
        ...
        try:
            ...
            self.client.upsert(collection_name="documents", points=sample_points)
    ...
    async def search_test(self) -> None:
        ...
        try:
            ...
            results = self.client.search(
                ...
                query_filter={
                    "must": [{"key": "category", "match": {"value": "sample"}}]
                },
                limit=5,
            )
...
async def recreate_collection(collection_name: str):
    ...
    confirm = input(
        f"This will DELETE all data in '{collection_name}'. Continue? (yes/no): "
    )
