...
class UserDomainService:
    ...
    async def authenticate_user(
        self, email: str, password: str, update_login_info: bool = True
    ) -> UserEntity:
    ...
    async def reset_user_password(self, email: str) -> Tuple[UserEntity, str]:
    ...
    async def promote_user_role(
        self, user_id: int, new_role: UserRole, promoted_by_user_id: int
    ) -> UserEntity:
    ...
    async def deactivate_user(
        self, user_id: int, deactivated_by_user_id: int, reason: Optional[str] = None
    ) -> UserEntity:
    ...
    async def cleanup_inactive_users(self, inactive_days: int = 90) -> int:
        ...
        inactive_users = await self.user_repository.get_unverified_users(
            created_before=cutoff_date, limit=1000
        )
    ...
    async def validate_user_action(
        self, user_id: int, action: str, resource_id: Optional[str] = None
    ) -> bool:
    ...
    async def _validate_registration_rules(
        self, role: UserRole, invited_by_user_id: Optional[int]
    ) -> None:
        ...
        if invited_by_user_id:
            ...
            if not inviter or inviter.role != UserRole.ADMIN:
                raise BusinessRuleViolationError("Only admins can invite new users")

    async def _validate_role_promotion(
        self, user: UserEntity, new_role: UserRole, promoter: UserEntity
    ) -> None:
    ...
    async def _validate_user_deactivation(
        self, user: UserEntity, deactivator: UserEntity
    ) -> None:
        ...
        if user.role == UserRole.ADMIN and user.id != deactivator.id:
            raise BusinessRuleViolationError("Admins cannot deactivate other admins")
