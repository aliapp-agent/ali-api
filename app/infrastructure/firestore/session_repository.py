...
class FirestoreSessionRepository(BaseFirestoreRepository, SessionRepositoryInterface):
    ...
    async def get_popular_sessions(
        ...
        query = self.collection.where('updated_at', '>=', cutoff_date)\
    ...
    async def get_session_statistics(
        ...
        # Calculate averages
        if len(docs) > 0:
            stats['average_messages_per_session'] = round(stats['total_messages'] / len(docs), 2)
        
        if sessions_with_response_time > 0:
            stats['average_response_time'] = round(total_response_time / sessions_with_response_time, 2)
    ...
    async def cleanup_deleted_sessions(
        ...
        """
        query = self.collection.where('status', '==', 'deleted')\
