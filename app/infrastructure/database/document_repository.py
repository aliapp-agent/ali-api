...
class PostgresDocumentRepository(DocumentRepositoryInterface):
    ...
    async def get_by_id(self, document_id: str) -> Optional[DocumentEntity]:
        ...
        try:
            ...
            logger.error("get_document_by_id_failed", error=str(e), document_id=document_id)
    ...
    async def update(self, document: DocumentEntity) -> DocumentEntity:
        ...
        try:
            ...
            logger.info("document_updated", document_id=document.id, title=document.title)
            ...
            self.db_session.rollback()
            logger.error("document_update_failed", error=str(e), document_id=document.id)
    ...
    async def get_user_documents(
        ...
        try:
            ...
            if status:
                ...
                query = query.where(DocumentModel.status != DocumentStatus.DELETED.value)
            
            if category:
                query = query.where(DocumentModel.category == category.value)
            
            # Apply pagination and ordering
            query = query.offset(offset).limit(limit).order_by(DocumentModel.updated_at.desc())
            ...
            raise RepositoryError(f"Database error during user documents lookup: {str(e)}")
    ...
    async def get_public_documents(
        ...
        try:
            ...
            raise RepositoryError(f"Database error during public documents lookup: {str(e)}")
    ...
    async def get_documents_by_hash(self, content_hash: str) -> List[DocumentEntity]:
        ...
        try:
            ...
            logger.error("get_documents_by_hash_failed", error=str(e), content_hash=content_hash)
    ...
    async def count_user_documents(
        ...
        """
        try:
            query = select(func.count(DocumentModel.id)).where(DocumentModel.user_id == user_id)
    ...
    async def get_document_statistics(
        ...
        try:
            ...
            draft_documents = self.db_session.exec(
                select(func.count(DocumentModel.id)).where(
                    and_(
                        DocumentModel.status == DocumentStatus.DRAFT.value,
                        *filters
                    )
                )
            ).one()
            ...
            error_documents = self.db_session.exec(
                select(func.count(DocumentModel.id)).where(
                    and_(
                        DocumentModel.status == DocumentStatus.ERROR.value,
                        *filters
                    )
                )
            ).one()
            ...
            avg_stats = self.db_session.exec(
                select(
                    func.avg(DocumentModel.metadata["word_count"].astext.cast(text("integer"))),
                    func.avg(DocumentModel.metadata["character_count"].astext.cast(text("integer"))),
                    func.avg(DocumentModel.metadata["file_size"].astext.cast(text("integer")))
                ).where(and_(*filters))
            ).first()
        ...
        try:
            ...
            result = self.db_session.exec(
                ...
                """.format(
                    ...
                    if exclude_user_ids else ""
                )),
                {
                    "older_than": older_than,
                    "updated_at": datetime.utcnow(),
                    **({"exclude_user_ids": tuple(exclude_user_ids)} if exclude_user_ids else {})
                }
            )
    ...
    def _entity_to_model(self, entity: DocumentEntity) -> DocumentModel:
        ...
        if entity.metadata:
            metadata_dict = {
                ...
                "processed_at": entity.metadata.processed_at.isoformat() if entity.metadata.processed_at else None,
    ...
    def _model_to_entity(self, model: DocumentModel) -> DocumentEntity:
        ...
        content = DocumentContent(
            raw_text=model.content or "",
            processed_text=model.content or "",
            chunks=[]
        )
        ...
        metadata = DocumentMetadata(
            ...
            processed_at=datetime.fromisoformat(metadata_data.get("processed_at")) if metadata_data.get("processed_at") else None,
    ...
    def _update_model_from_entity(self, model: DocumentModel, entity: DocumentEntity) -> None:
