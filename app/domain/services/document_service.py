...
class DocumentDomainService:
    ...
    async def create_document(
        ...
        if not user.is_active:
            raise BusinessRuleViolationError("Inactive users cannot create documents")
    ...
    async def publish_document(self, document_id: str, user_id: int) -> DocumentEntity:
    ...
    async def detect_duplicate_documents(
        self, user_id: Optional[int] = None, similarity_threshold: float = 0.9
        ...
        if user_id:
            ...
            documents = await self.document_repository.get_public_documents(limit=10000)
    ...
    async def archive_old_documents(
        ...
        count = await self.document_repository.archive_old_documents(
            older_than=cutoff_date, exclude_user_ids=exclude_user_ids
        )
            ...
            try:
                ...
                if not document.can_be_edited_by(user_id, user.role.value):
                    ...
                    results["errors"].append(
                        f"No edit access to document {document_id}"
                    )
                    continue
                ...
                if operation == "archive":
                    ...
                    if category:
                        ...
                        results["failed"] += 1
                        results["errors"].append(
                            f"No category provided for {document_id}"
                        )
                        continue
                else:
                    results["failed"] += 1
    ...
    async def _get_document_with_edit_access(
        self, document_id: str, user_id: int
    ) -> DocumentEntity:
    ...
    async def _validate_document_size(
        self, content: DocumentContent, user_role: str
    ) -> None:
        ...
        size_limits = {"admin": 100.0, "editor": 50.0, "viewer": 10.0, "guest": 5.0}
    ...
    async def _validate_document_type(
        self, document_type: DocumentType, user_role: str
    ) -> None:
    ...
    async def _check_document_quotas(self, user_id: int, user_role: str) -> None:
        ...
        quotas = {"admin": 10000, "editor": 1000, "viewer": 100, "guest": 10}

        max_documents = quotas.get(user_role, 10)

        # Count user's documents
        document_count = await self.document_repository.count_user_documents(
            user_id=user_id, status=DocumentStatus.ACTIVE
        )

        if document_count >= max_documents:
            raise QuotaExceededError(
                quota_type="user_documents", used=document_count, limit=max_documents
            )

    async def _check_duplicate_content(
        self, content: DocumentContent, user_id: int
    ) -> None:
        ...
        duplicate_docs = await self.document_repository.get_documents_by_hash(
            content_hash
        )
    ...
    async def _validate_document_publication(self, document: DocumentEntity) -> None:
        ...
        if not document.content.raw_text.strip():
            raise BusinessRuleViolationError("Cannot publish document without content")

        # Check if document has title
        if not document.title.strip():
            raise BusinessRuleViolationError("Cannot publish document without title")

        # Check if document is in valid status
        if document.status == DocumentStatus.ERROR:
            raise BusinessRuleViolationError("Cannot publish document with errors")
    ...
    async def _get_category_distribution(self, user_id: Optional[int]) -> Dict:
        ...
        return {"lei": 45, "decreto": 23, "relatorio": 15, "other": 12}
