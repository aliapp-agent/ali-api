...
from app.core.firebase import get_firestore, get_firebase_auth, get_storage
...
class FirebaseMigrationService:
    ...
    async def migrate_sessions(self) -> None:
        ...
        for session_row in sessions:
            try:
                ...
                if not firebase_uid:
                    logger.warning(f"User {session_row.user_id} not found in Firebase, skipping session {session_row.id}")
                    continue
        ...
        try:
            ...
            for table_name in ['chat_message', 'message']:
                try:
                    ...
                    if messages:
                        logger.info(f"Found {len(messages)} messages in {table_name} to migrate")
        ...
        for message_row in messages:
            try:
                if not message_row.session_exists:
                    logger.warning(f"Session {message_row.session_id} not found, skipping message {message_row.id}")
                    continue
                
                # Add message to session subcollection
                session_ref = self.firestore_db.collection('chat_sessions').document(str(message_row.session_id))
                ...
                if self.migration_stats['messages_migrated'] % 100 == 0:
                    logger.info(f"✅ Migrated {self.migration_stats['messages_migrated']} messages...")
    ...
    async def migrate_documents(self) -> None:
        ...
        try:
            ...
            logger.info("Document migration not implemented yet - manual migration required")
    ...
    def print_migration_stats(self) -> None:
        ...
        logger.info("="*50)
        logger.info(f"Users: {self.migration_stats['users_migrated']} migrated, {self.migration_stats['users_failed']} failed")
        logger.info(f"Sessions: {self.migration_stats['sessions_migrated']} migrated, {self.migration_stats['sessions_failed']} failed")
        logger.info(f"Messages: {self.migration_stats['messages_migrated']} migrated, {self.migration_stats['messages_failed']} failed")
        logger.info(f"Documents: {self.migration_stats['documents_migrated']} migrated, {self.migration_stats['documents_failed']} failed")
        logger.info("="*50)
