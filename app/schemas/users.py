...
class UserPreferences(BaseModel):
    ...
    notifications_browser: bool = Field(
        True, description="Enable browser notifications"
    )
...
class UserProfile(BaseModel):
    ...
    organization: Optional[str] = Field(
        None, description="Organization", max_length=100
    )
    ...
    linkedin: Optional[str] = Field(
        None, description="LinkedIn profile", max_length=100
    )
...
class UserSearchRequest(BaseModel):
    ...
    last_login_from: Optional[datetime] = Field(
        None, description="Filter by last login from"
    )
    last_login_to: Optional[datetime] = Field(
        None, description="Filter by last login to"
    )
...
class UserBulkUpdate(UserBulkOperation):
    ...
    role: Optional[UserRole] = Field(
        None, description="New role for change_role operation"
    )
    permissions: Optional[List[Permission]] = Field(
        None, description="Permissions for add/remove operations"
    )
...
class UserPermissionResponse(BaseModel):
    ...
    additional_permissions: List[Permission] = Field(
...
class UserInvitation(BaseModel):
    ...
    message: Optional[str] = Field(
        None, description="Invitation message", max_length=500
    )
